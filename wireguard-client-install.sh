...
function readClientConfigFromTerminal() {
	CLIENT_CONFIG="$(mktemp)"

	echo ""
	echo "Paste the full WireGuard client .conf content from the server installer."
	echo "Include everything from [Interface] through [Peer] (keys, endpoint, AllowedIPs)."
	echo ""
	echo -e "${ORANGE}When finished, press Ctrl+D on a new empty line.${NC}"
	echo ""

	if ! cat >"${CLIENT_CONFIG}"; then
		echo -e "${RED}Failed to read client config from terminal input.${NC}"
		exit 1
	fi

	if [[ ! -s "${CLIENT_CONFIG}" ]]; then
		echo -e "${RED}No config content received.${NC}"
		exit 1
	fi
}

function writeClientConfigFromContent() {
	local content=$1
...
function resolveClientConfig() {
	...
	if isAutoInstall; then
		...
		elif [[ -n "${WG_CONF_CONTENT}" ]]; then
			writeClientConfigFromContent "${WG_CONF_CONTENT}"
		else
			echo -e "${RED}AUTO_INSTALL requires WG_CONF_FILE, WG_CONF_URL, or WG_CONF_CONTENT.${NC}"
			exit 1
		fi
